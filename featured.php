...
if($get_total = $mysqli->query("SELECT * FROM ads WHERE active=1 AND feat=1")){
...
<li><a href="featured-recent-1.html">All Ads</a></li>
...
<span class="strong-label-top pull-left">Sort By:</span>&nbsp
 <div class="dropdown btn-dropdown">
    ...
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
      <li role="presentation"><a role="menuitem" tabindex="-1" href="featured-recent-1.html">Most Recent</a></li>
      <li role="presentation"><a role="menuitem" tabindex="-1" href="featured-price-1.html">Lowest Price</a></li>
    </ul>
  </div>
...
// How many adjacent pages should be shown on each side?
	...
	$query = $mysqli->query("SELECT COUNT(*) as num FROM ads WHERE active=1 AND feat=1 ORDER BY id DESC");
	...
	if($page) 
 	...
 	/* Get data. */
	$result = $mysqli->query("SELECT * FROM ads WHERE active=1 AND feat=1 ORDER BY id DESC LIMIT $start, $limit");
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= "<li><a href=\"featured-recent-$prev.html\">&laquo;</a></li>";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= "<li><a href=\"featured-recent-$counter.html\">$counter</a></li>";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= "<li><a href=\"featured-recent-$lpm1.html\">$lpm1</a></li>";
				$pagination.= "<li><a href=\"featured-recent-$lastpage.html\">$lastpage</a></li>";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= "<li><a href=\"featured-recent-1.html\">1</a></li>";
				$pagination.= "<li><a href=\"featured-recent-2.html\">2</a></li>";
				$pagination.= "...";
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= "<li><a href=\"featured-recent-$next.html\">&raquo;</a></li>";
		else
			$pagination.= "<li class=\"disabled\"><a href=\"#\">&raquo;</a></li>";
		$pagination.= "</ul>\n";		
	}
	
	$query= $mysqli->query("SELECT * FROM ads WHERE active=1 AND feat=1 ORDER BY id DESC limit $start,$limit");
...
// How many adjacent pages should be shown on each side?
	...
	$query = $mysqli->query("SELECT COUNT(*) as num FROM ads WHERE active=1 AND feat=1 ORDER BY price ASC");
	...
	if($page) 
		...
		$start = 0;								//if no page var is given, set start to 0
 	/* Get data. */
	$result = $mysqli->query("SELECT * FROM ads WHERE active=1 AND feat=1 ORDER BY price ASC LIMIT $start, $limit");
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= "<li><a href=\"featured-price-$prev.html\">&laquo;</a></li>";
		else
			$pagination.= "<li class=\"disabled\"><a href=\"#\">&laquo;</a></li>";	
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= "<li><a href=\"featured-price-$counter.html\">$counter</a></li>";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= "<li><a href=\"featured-price-$lpm1.html\">$lpm1</a></li>";
				$pagination.= "<li><a href=\"featured-price-$lastpage.html\">$lastpage</a></li>";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= "<li><a href=\"featured-price-1.html\">1</a></li>";
				$pagination.= "<li><a href=\"featured-price-2.html\">2</a></li>";
				$pagination.= "...";
				for ($counter = $page - $adjacents; $counter <= $page + $adjacents; $counter++)
				{
					if ($counter == $page)
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= "<li><a href=\"featured-price-$next.html\">&raquo;</a></li>";
		else
			$pagination.= "<li class=\"disabled\"><a href=\"#\">&raquo;</a></li>";
		$pagination.= "</ul>\n";		
	}
	
	$query= $mysqli->query("SELECT * FROM ads WHERE active=1 AND feat=1 ORDER BY price ASC limit $start,$limit");
